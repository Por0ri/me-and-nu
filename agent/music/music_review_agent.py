# # music_review_agent_v3.4 — 음악 리뷰 에이전트
#
...
def 편집국장_읽기(글: Article, 개요_: 개요, 재료, 관점표, 맥락, album, budget, 아티스트말=None, 앞판=None, 차례=None, 종류=""):
    if budget["남은콜"] <= 0:
        return None
...
def n_편집국장(st: RunState) -> RunState:
    ...
    st.판정 = 편집국장_읽기(st.글, st.개요_, st.재료, st.관점표, st.맥락, st.album, b, 아티스트말=st.아티스트말, 앞판=앞판, 차례=st.바퀴, 종류=종류)
    st.남은콜 = b["남은콜"]
