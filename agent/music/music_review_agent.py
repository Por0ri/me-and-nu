...
# - (9/22) 편집국장이 앞 판 판정을 받는다(앞판기억). 앞 판 점수 · 문제 목록을 보고 문제마다 고쳐짐 / 남음 / 새로 생김을 표시한다.
#   점수는 앞 점수를 기준으로 낸다. 고쳐진 게 있고 새 문제가 없으면 앞 점수 아래로 안 내린다. 내려가면 점수설명에 왜인지 적는다.
#   전에는 판마다 처음 보는 것처럼 채점해서 같은 글도 점수가 흔들렸다. 올랐는지 내렸는지 판단할 근거가 없었다.
#
# v3.4에서 바뀐 것
# - 편집국장이 앞 판 글 본문도 같이 받는다. 앞 판 판정만 받으면 이번 글을 처음 보는 새 글로 읽는다.
#   [앞 판 글]과 [이번 글]을 나란히 주고, 이번 글이 앞 글을 고친 것인지 · 처음부터 다시 쓴 것인지(판 종류)를 적어 준다.
#   앞 판에서 안 잡았던 문제를 이번에 잡으면 "새로 생김"이 아니라 "앞 판에서 못 봄"으로 적는다. 점수를 내린 이유가 글 탓인지 편집국장 탓인지 갈라 보려는 것이다.
# - 기록마다 판 종류를 남긴다. 첫 판 / 새로 씀. (음악은 편집국장이 걸면 기획자부터 다시 가므로 고침은 없다)
# - 마지막 판이 탈락이면 기록 중 점수가 제일 높은 판(최고 판)의 글을 저장한다. 다시 쓰다 나빠진 글이 남지 않게 한다.
#   설정값 최고판저장. False면 마지막 판을 저장한다(v3.3과 같다).
# - 같은 방식을 애니 v0.6 · 영화 정보 V2.4 · 영화 리뷰 V1.9에도 넣었다. 판정관 · 편집국장이 앞 판을 기억하는 방식은 넷이 같다.
#
# v3.5에서 바뀐 것 — 글이 단조롭다 (PM). 애니 v0.3에서 고친 프롬프트 방식을 음악에 옮겼다
# - 온도 · 시작점 · 배치 · 접근이 낱말로만 가던 것을 뜻글로 준다(온도글 · 시작점글 · 배치글 · 접근글 · 주문_text).
#   기획자 · 작가 · 편집국장이 같은 [주문]을 받는다. 전에는 "[온도] 짓궂음" 한 줄뿐이라 짓궂은 데가 하나도 안 나왔다.
# - 작가에게 작가_공통(쓰는 사람 · 문체 · 글이 이어지게 · 문단 여는 법 · 네 판단 · 숫자 · 독자 앞에는 표가 없다)을 준다. 전에는 금지 목록뿐이었다.
#   비유는 둔다(PM). 애니의 비유 금지는 안 옮겼다.
# - 편집국장이 [주문대로 갔는가]와 [되풀이 — 글의 꼴](같은 꼴 문단 · 같은 길이 문장 · 같은 방식으로 문단 열기)을 본다.
# - 형태검사(코드)에 문장 길이 단조 · 곡 이름으로 문단 열기 · 아티스트 표기 섞임 · 곡 표기 병기 없음.
# - 표기표. 아티스트 대표 표기는 앨범 정보의 이름(MusicBrainz 이름, WOODZ)이고 다른 표기(우즈)는 첫 등장에 괄호로 한 번(표기 · 표기짝).
#   곡은 MusicBrainz 릴리즈 여러 개의 곡 제목을 모아 짝을 만든다(곡짝: 심연 = ABYSS). 수록곡 표기가 대표, 짝은 첫 등장에 괄호로 한 번.
#   편집국장은 짝 안의 표기를 사실 문제로 안 잡는다. 전에는 '심연'을 세 번 잡았는데 무게 규칙이 "표기"라는 낱말로 사소 처리해 94점으로 올라갔다.
# - 판종 문자열 "EP·미니앨범"을 "EP"로. 작가가 그 덩어리를 문장에 그대로 썼다. 앨범 정보 표시만 "EP (미니앨범)"이다.
...
# v3.5
조건돌려쓰기   = True   # run에서 편마다 접근 · 온도 · 시작점을 돌려 쓴다. False면 편마다 무작위(v3.4까지)
곡짝릴리즈수   = 3      # MusicBrainz 릴리즈 몇 개까지 곡 제목을 더 읽어 짝(심연 = ABYSS)을 만들지. 하나에 1초쯤 든다
문장길이편차문턱 = 0.28  # 문장 길이의 편차 / 평균이 이 아래면 "문장 길이 단조"로 건다. 0이면 안 건다
# ─────────────────────────────────────────────────────────────
...
def 표기짝(album):
    # v3.5: (대표 표기, 다른 표기들). 대표는 앨범 정보의 아티스트 이름이다. MusicBrainz에서 왔으면 그쪽 이름(WOODZ),
    # 매체에서만 왔으면 매체 표기다. 다른 표기는 한글 · 영문 별칭 중 대표와 다른 것이다
    별 = album.get("별칭") or {}
    대표 = album.get("artist") or 별.get("영문") or 별.get("한글") or ""
    다른 = [x for x in (별.get("한글"), 별.get("영문")) if x and _norm(x) != _norm(대표)]
    return 대표, list(dict.fromkeys(다른))


def 표기(album):
    # 글에 쓰는 표기. "WOODZ(우즈)" 꼴. 대표 표기 뒤에 다른 표기를 괄호로 한 번 붙인다. 뒤로는 대표만 쓴다
    대표, 다른 = 표기짝(album)
    return f"{대표}({다른[0]})" if 다른 else 대표


def 곡표기_text(album):
    # v3.5: 곡 표기 짝. "심연 = ABYSS" 한 줄씩. 없으면 빈 문자열
    return chr(10).join(f"{k} = {' / '.join(v)}" for k, v in (album.get("곡짝") or {}).items())
...
def album_detail(album):
    ...
    판종 = []
    if d.get("primary-type") == "EP": 판종.append("EP")
    ...
    if first:
        try:
            ...
            out["수록곡"] = []
        # v3.5: 다른 릴리즈의 곡 제목. 같은 자리 제목이 다르면 짝으로 둔다 (한국 릴리즈 '심연' = 다른 릴리즈 'ABYSS')
        for rel in [x for x in rels if x.get("id") != first["id"]][:곡짝릴리즈수]:
            if not out["수록곡"]:
                break
            try:
                r2 = mb_get(f"release/{rel['id']}", inc="recordings")
                다른 = [t["title"] for m in r2.get("media", []) for t in m.get("tracks", [])]
                곡짝_더하기(out["곡짝"], out["수록곡"], 다른)
            except Exception:
                pass
    else:
        out["수록곡"] = []
...
def 곡짝_더하기(짝표, 수록곡, 다른):
    # 같은 자리(순서)의 제목이 다르면 짝이다. 곡 수가 다르면 겹치는 앞부분만 본다
    for a, b in zip(수록곡, 다른):
        if a and b and _norm(a) != _norm(b) and b not in 짝표.setdefault(a, []):
            짝표[a].append(b)
    for k in [k for k, v in 짝표.items() if not v]:
        del 짝표[k]
    return 짝표
...
def 매체정보로_앨범(cand):
    ...
    if re.search(r"\bep\b|미니", low): 판종.append("EP")
...
def album_text(a):
    ...
    판종 = a.get("판종") or "모름"
    if 판종.startswith("EP"):
        판종 = 판종.replace("EP", "EP (미니앨범)", 1)          # 글에는 EP라고만 쓴다. 미니앨범은 뜻이다
    return (f"아티스트: {표기(a)}  (대표 표기 {대표}. 다른 표기: {', '.join(다른) or '없음'}. 글에서는 첫 등장에 {표기(a)}, 그 뒤로는 {대표})\n"
            f"앨범: {a['title']} ({a.get('year')})\n"
            f"판종: {판종}{(' — ' + a['판설명']) if a.get('판설명') else ''}\n"
            f"발매일: {a.get('발매일') or '모름'}\n"
            f"레이블: {', '.join(a.get('레이블') or []) or '모름'}\n"
            f"수록곡: {', '.join((a.get('수록곡') or [])[:25]) or '모름'}\n"
            f"곡 표기 짝 (앞이 대표. 첫 등장에 괄호로 한 번 붙인다): {곡표기_text(a).replace(chr(10), ' · ') or '없음'}")
...
접근글 = {
"분석": "이 앨범이 어떻게 만들어졌는지를 본다. 소리 · 편곡 · 곡 배치가 무엇을 어떻게 하는지. 좋다 · 나쁘다는 뒤로 미룬다. 판단은 마지막에 한 번이다.",
"해석": "이 앨범이 무엇을 말하는지를 본다. 가사 · 제목 · 곡 순서가 가리키는 것. 네 해석은 근거 곡과 같은 문단에 있다. 근거 없는 해석은 쓰지 않는다.",
"평가": "이 앨범이 되는지 안 되는지를 본다. 첫 문단에서 마지막 문단까지 네 판단이 글의 중심이다. 왜 그런지가 같은 문장에 있어야 한다. 평론의 판단 뒤에 숨지 않는다.",
}

온도글 = {
"건조": """[온도 — 건조]
감정을 드러내지 않는다. 소리 · 가사 · 연도 · 사람 이름과 네 관찰만 놓는다.
좋다 · 싫다 대신 된다 · 안 된다, 들린다 · 안 들린다로 말한다. "아깝다", "반갑다"를 쓰지 않는다.
웃긴 대목이 있어도 웃기다고 말하지 않는다. 그 대목만 보여주고 지나간다.
판단은 그래도 한다. 건조는 판단이 없다는 뜻이 아니라 판단에 감정을 안 붙인다는 뜻이다.""",
"따뜻함": """[온도 — 따뜻함]
이 앨범을 아끼는 게 보인다. 놀라거나 감탄해도 된다. 다만 왜 그런지가 같은 문장에 있어야 한다.
  안 됨: 'Drowning'은 정말 좋은 곡이다.
  됨: 'Drowning'은 보컬을 겹겹이 쌓아서 숨을 쉬려 할수록 더 잠기는 소리를 만든다. 그래서 앨범 가운데에서 한 번 멈추게 된다.
아쉬운 곡을 말할 때도 깎는 게 아니라 아까워하는 쪽이다. 다만 아쉽다는 말을 빼먹지는 않는다.
이 앨범을 처음 트는 사람을 생각하며 쓴다. 어느 곡에서 무엇이 낯설지를 미리 말해 준다.
아끼는 마음을 "명반", "레전드"로 줄이지 않는다. 어느 곡 어느 대목 때문인지로 말한다.""",
"짓궂음": """[온도 — 짓궂음]
정확하게 들어서 웃긴 것을 놓치지 않는다. 한 편에 한두 군데다. 세 번째부터는 가벼워진다.
  비꼼: 일곱 곡에 장르가 일곱 개라니 대단하다.
  짓궂음: 일곱 곡 안에서 R&B로 눕혔다가 록으로 일으켰다가 다시 눕히는데, 그렇게 도착한 마지막 곡 제목이 '심연'이다.
비꼬지 않는다. 앨범이 스스로 드러낸 우스운 대목을 그대로 보여주는 것이지 앨범을 깎는 게 아니다.
사람을 놀려서 웃기지 않는다. 아티스트 · 팬 · 평론가를 놀려서 웃기지 않는다.
웃긴 대목도 사실 규칙 안에서다. 재료 밖의 것으로 웃기지 않는다.
농담을 한 뒤에 설명하지 않는다. 웃긴 문장은 그냥 두고 지나간다.""",
}

시작점글 = {
"곡 하나의 특정 대목": "곡 하나의 한 대목(도입부의 악기, 후렴 앞의 멈춤, 마지막 아웃트로)을 첫 문단에 놓고 거기서 시작한다. 그 곡이 앨범에서 몇 번째인지는 그 다음이다. 마지막 문단에서 그 대목을 한 번 다시 부르며 닫는다.",
"앨범이 나온 때의 자리": "이 앨범이 나온 해에 아티스트가 어디에 있었는지(앞 앨범 뒤 몇 해, 소속 · 활동 상황)에서 시작한다. 맥락에 연도와 같이 적힌 것만 쓴다. 그 자리가 앨범의 소리를 어떻게 정했는지가 글 전체다.",
"아티스트의 앞 앨범": "앞 앨범(또는 앞 곡)과 이 앨범이 어디서 갈라지는지에서 시작한다. 맥락 · 평론에 앞 앨범 이야기가 있을 때만 쓸 수 있다. 없으면 이 앨범 안의 앞 곡과 뒤 곡으로 대신한다. 마지막에 그 갈림이 무엇을 남겼는지로 닫는다.",
"평이 갈린 지점": "같은 곡 · 같은 대목을 두고 평이 갈린 자리에서 시작한다. 무엇을 두고 갈리는지를 보이고, 네가 어느 쪽에 서는지와 왜 그런지가 글 전체다. 평이 하나뿐이면 그 평과 네 귀가 갈리는 자리다.",
"소리의 한 요소": "소리 하나(보컬을 겹쳐 쌓는 법, 드럼의 빠르기, 기타 톤, 편곡의 빈자리)를 첫 문장에 놓는다. 그 요소가 앨범 안에서 어디에 나오고 어디에 안 나오는지를 좇는다. 마지막에 그 요소가 앨범을 어떻게 정했는지로 닫는다.",
"앨범 제목이나 표지": "앨범 제목(빈칸 · 철자 · 뜻)이나 표지에서 시작한다. 그것이 곡 어디에서 소리로 나타나는지를 좇는다. 제목 풀이로 끝내지 않는다. 마지막 문단에서 제목을 한 번 다시 부르며 닫는다.",
"듣는 사람이 서는 자리": "이 앨범을 처음 듣는 사람이 어디서 멈추는지(첫 곡의 첫 소리, 장르가 바뀌는 곡, 마지막 곡)에서 시작한다. 그 멈춤이 무엇 때문인지를 풀어 주는 것이 글이고, 풀린 자리가 마지막 문단이다.",
}

배치글 = {
"시간 순으로 따라간다": "앨범 앞뒤의 시간을 따라간다. 앞 앨범 → 이 앨범 → 나온 뒤. 맥락에 연도가 있을 때만 고른다. 곡은 시간 안에서 나온다. 문단마다 시점이 다르다.",
"곡 하나를 파고든 다음 앨범 전체로 넓힌다": "곡 하나를 두세 문단 동안 끝까지 판다. 소리 · 가사 · 앨범 안의 자리. 그 다음 앨범 전체가 그 곡을 어떻게 둘러싸는지로 넓힌다. 곡셋은 그 곡과 둘까지다.",
"평이 갈린 지점에서 시작해 양쪽을 본다": "갈린 자리를 첫 문단에 놓고, 한쪽 근거 → 다른 쪽 근거 → 네 자리로 간다. 문단마다 어느 편인지 분명하다. 마지막에 네가 서는 자리와 왜 그런지다.",
"소리 요소별로 나눠 본다": "보컬 · 리듬 · 편곡처럼 소리 요소를 하나씩 문단으로 한다. 문단마다 요소 하나, 그 요소가 두드러지는 곡 하나. 마지막에 요소들이 합쳐 무엇이 되는지다.",
"앨범 전체 인상을 먼저 말하고 근거를 댄다": "첫 문단에 네 판단 한 줄. 나머지 문단이 그 판단을 곡으로 받친다. 마지막 문장은 첫 판단으로 돌아오되 같은 문장을 다시 적지 않는다.",
"곡 순서를 따라간다": "1번 곡부터 차례로 간다. 곡마다 무슨 일이 있는지와 다음 곡으로 어떻게 넘어가는지. 곡마다 같은 꼴(곡 이름 → 소리 → 감정 → 다음 곡)로 쓰지 않는다. 어떤 곡은 한 문장으로 지나가고 어떤 곡은 한 문단을 준다. 이어짐은 재료에 있을 때만 적는다.",
}


def 주문_text(접근, 온도, 시작점, 배치=None):
    # v3.5: 기획자 · 작가 · 편집국장이 같은 것을 받는다. 낱말만 넘기지 않고 뜻까지 넘긴다
    줄 = [f"접근 {접근} · 온도 {온도} · 시작점 {시작점}" + (f" · 배치 {배치}" if 배치 else ""),
          f"[접근 — {접근}]" + chr(10) + 접근글.get(접근, ""),
          온도글.get(온도, f"[온도 — {온도}]").strip(),
          f"[시작점 — {시작점}]" + chr(10) + 시작점글.get(시작점, "")]
    if 배치:
        줄.append(f"[배치 — {배치}]" + chr(10) + 배치글.get(배치, ""))
    return (chr(10) * 2).join(줄)


def 배치후보_text(피할=()):
    줄 = []
    for b in 배치들:
        표시 = "  (앞 편에서 썼다. 이번엔 피한다)" if b in (피할 or ()) else ""
        줄.append(f"- {b}{표시}: {배치글[b]}")
...
class 기획주문(BaseModel):
    ...
    평개수: int = 0
    지난문제: str = ""
    피할배치: List[str] = Field(default_factory=list)   # v3.5: 앞 편이 쓴 배치. 기획자가 피한다
...
{주문_text(d.접근, d.온도, d.시작점)}{중심}

[배치 후보 — 하나 고른다. 이름 그대로 적는다]
{배치후보_text(d.피할배치)}

[배치 고르기]
- 재료가 앨범을 하나의 흐름으로 말하면(곡 순서, 앞뒤 곡의 이어짐, 앨범 전체의 이야기를 평론이 적어 놓았으면) "{곡순서}"를 골라도 된다. 다만 앞 편이 썼으면 피한다.
- 재료가 곡 하나하나를 안 다루면 "{곡순서}"는 고르지 않는다. 이어짐을 지어낼 수 없다.
- 시작점과 맞는 배치를 고른다. "평이 갈린 지점"에서 열면서 "곡 순서"로 가지 않는다.

[문단 계획 — 뼈대가 글의 꼴을 정한다]
- 할말은 첫 문장이 아니다. 작가가 그대로 옮기지 않게 판단 · 장면 · 질문 · 소리를 섞어 적는다. 문단마다 할말의 꼴이 다르다.
- 문단마다 같은 꼴(곡 이름 → 소리 → 감정)이 되지 않게 한다. 어떤 문단은 곡 하나를 끝까지 가고, 어떤 문단은 두 곡을 마주 놓고, 어떤 문단은 곡 없이 앨범 자리(연도 · 앞 앨범 · 제목)를 말한다.
- 첫 문단은 시작점대로 연다. 발매일 · 레이블 · 곡 수로 열지 않는다. 그 사실은 필요한 문단에 나눠 넣는다.
...
- 곡셋은 둘 이상이다. 곡메모나 수록곡에 있는 곡에서 고른다. 곡메모가 있는 곡을 먼저 고른다. 곡 이름은 앨범 정보의 수록곡 표기 그대로(곡 표기 짝이 있으면 앞 이름). 영어 곡을 한국어로 옮기지 않는다.
...
class 집필주문(BaseModel):
    ...
    접근: str = ""            # v3.5
    시작점: str = ""          # v3.5
    곡표기: str = ""          # v3.5: 곡 표기 짝. "심연 = ABYSS"

class Article(BaseModel):
    제목: str
    본문: str

작가_공통 = """너는 앨범 리뷰를 쓴다. 뼈대(개요)를 받지만 그것은 채워야 할 칸이 아니다. 무엇을 어떤 차례로 말할지의 메모다. 문장은 네가 만든다.

[쓰는 사람]
음악을 오래 많이 들은 사람이 쓴다. 이 아티스트가 처음이 아니다.
설명하지 않고 말한다. 독자가 이 앨범을 모른다고 해서 하나하나 풀어 주지 않는다.
  풀어 줌: 《OO-LI》는 2023년에 나온 WOODZ의 다섯 번째 미니앨범이다.
  말함: 《OO-LI》는 잠으로 열어서 잠으로 닫는데, 그 사이에 록 밴드가 한 번 지나간다.
무엇이 귀에 걸렸는지를 말한다. 놀라거나 감탄해도 된다. 다만 왜 그런지가 같은 문장에 있어야 한다.
유머는 관찰에서 나온다. 웃기려고 쓰지 않는다. 정확하게 들으면 웃긴 것이 들린다. 농담을 한 뒤에 설명하지 않는다.
자기 판단이 있다. 판단을 남에게 미루지 않는다. "평론가들은", "팬들은" 뒤에 숨지 않는다.
싸우지 않는다. 아티스트 · 프로듀서 · 팬을 사람으로 깎지 않는다.
...
문장 길이는 내용이 정한다.
구체적인 것 하나를 말할 때는 짧다. "'Drowning'은 드럼으로 연다."
왜 그런지를 따라갈 때는 길다. 따라가는 동안 끊지 않는다.
  토막: 'Drowning'은 록이다. 보컬이 겹쳐 있다. 숨이 막힌다.
  이음: 'Drowning'이 숨 막히게 들리는 것은 록 밴드 소리 위에 보컬을 겹겹이 쌓아서, 숨을 쉬려 할수록 더 깊이 잠기는 상태를 소리로 만들기 때문이다.
모든 문장을 같은 길이로 쓰지 않는다. 같은 꼴의 문장을 두 번 연달아 쓰지 않는다.
  같은 꼴: 'Busted'는 어쿠스틱으로 시작해 베이스가 들어온다. 'Who Knows'는 그런지로 시작해 멜로디가 부딪힌다. (표다. 글이 아니다)
...
"~에 가깝다", "~인 셈이다", "~할 만하다", "~는 편이다", "~일 수 있다", "~로 보인다"는 판단을 흐리는 말이다. 한 편에 한 번을 넘기지 않는다.
  흐림: 후반부는 전반부보다 록에 가까운 편이다.
  판단: 후반부는 록이다. 'Ready to Fight'가 드럼으로 열고 오케스트라까지 얹기 때문이다.

비유를 써도 된다. 다만 비유 뒤에 실제 소리가 따라와야 한다. 비유만으로 문단을 채우지 않는다.
...
곡 이름을 가운뎃점으로 셋 넘게 잇지 않는다. 쉼표나 "와"로 잇고, 셋 넘게 나열하지 않는다.

중학생이 읽어도 바로 이해되는 말로 쓴다.
어려운 말 대신 일상어를 쓴다. 서사 · 정체성 · 밀도 · 완성도 · 텍스처 · 층위 같은 말은 실제로 들리는 것으로 바꿔 쓴다.
사람이 말하는 것처럼 읽혀야 한다. 소리 내어 읽었을 때 어색하면 고친다.
이 프롬프트에 든 예시 문장을 그대로 옮기지 않는다. 예시는 모양을 보여주는 것이지 재료가 아니다.

[글이 이어지게 쓴다]
문단 계획의 할 말 · 곡 · 사실은 채워야 할 칸이 아니다. 하나의 글 안에서 서로를 부르며 나온다.
앞 문장이 다음 문장을 부른다. 앞 문장에서 꺼낸 것을 다음 문장이 받아서 이어 간다.
  안 됨: 《OO-LI》는 2023년 4월 26일에 나왔다. 일곱 곡이다. 레이블은 EDAM이다. (서로 상관없는 사실을 붙였다)
  됨: 《OO-LI》가 일곱 곡으로 록과 R&B를 다 지나갈 수 있는 것은 곡마다 장르를 바꾸는 게 아니라 곡 안에서 바꾸기 때문이다.
사실 하나를 말했으면 그 사실이 이 앨범에서 무엇을 뜻하는지 이어서 말한다.
곡 하나를 적으면 그 곡이 왜 여기 필요한지 한 문장 붙인다. "이어서", "그다음에는", "결국"으로 곡을 잇는 문장을 연달아 쓰지 않는다.
발매일 · 레이블 · 곡 수 · 결성 연도를 한 곳에 몰아 적지 않는다. 필요한 문단에 하나씩 나온다.
같은 사실을 같은 문단 안에서 두 번 말하지 않는다. 앞 문단이 말한 것을 뒤 문단이 다시 확인하지 않는다.
다 넣으려 하지 않는다. 짧은 글은 요약이 아니라 선택이다. 곡 하나를 한 문장으로 처리하느니 그 곡을 뺀다.
문단은 생각이 바뀌는 자리에서만 나눈다. 곡이 바뀐다고 나누지 않는다.
글 하나에 주제 하나, 흐름 하나다. 처음에 잡은 것이 끝까지 간다.

[문단 여는 법]
문단마다 시작하는 방식을 바꾼다. 같은 방식으로 두 문단 연속 시작하지 않는다.
- 소리를 그리며 시작한다. "드럼 두 번, 그리고 기타."
- 질문으로 시작한다. "그러면 이 앨범은 록 앨범인가."
- 흔한 말을 받아 뒤집으며 시작한다. "장르가 다양하다고들 하는데, 곡마다 다른 게 아니라 곡 안에서 바뀐다."
- 앞 문단 끝을 받아서 시작한다.
- 판단으로 시작할 때는 근거를 같은 문장 안에 붙인다.
곡 이름으로 문단을 여는 것은 한 편에 두 번까지다. 세 문단 넘게 곡 이름으로 열면 표다.
"X는 Y다"처럼 짧게 못 박는 문장으로 문단을 열지 않는다. "《OO-LI》는 일곱 곡 EP다"는 구호지 문장이 아니다.
《앨범명》이나 "이 앨범은"처럼 같은 주어로 문단을 두 번 연속 열지 않는다.
문단 계획의 "할 말" 한 줄을 첫 문장으로 그대로 옮기지 않는다.

[독자 앞에는 표가 없다]
앨범 정보 · 관점표 · 발췌 · 곡메모는 네가 본 재료다. 독자는 못 봤다. 그 이름을 부르지 않는다.
...
관찰은 네 몫이다. 어느 곡에서 소리가 바뀌는지, 앨범이 어디서 한 번 꺾이는지는 네가 듣고 네가 말한다. 다만 재료에 있는 것만이다. 에이전트는 음악을 못 듣는다. 곡 사이 이어짐 · 소리 묘사는 평론 · 곡메모에 있는 것만 쓴다.
근거 없이 세게 말하지 않는다. "역대급", "레전드", "명반", "놓치면 후회"를 쓰지 않는다.
들으라고 부추기지 않는다. "꼭 들어야", "추천한다", "필청"을 쓰지 않는다.
남의 평("평론가들은", "평단은")을 지어내지 않는다. 별점 · 차트 순위 · 조회수를 쓰지 않는다.

[숫자]
연도 · 곡 수 · 러닝타임은 그 숫자가 글에 필요할 때만 쓴다. 같은 숫자를 두 번 넘게 쓰지 않는다.
시간 감각은 말로 쓴다. "3년 만에"가 아니라 "세 해 만에"다. 발매일은 필요할 때 한 번이다."""
...
[앨범] {d.앨범한줄}
[아티스트 표기] {d.아티스트표기}
[곡 표기 짝 — 앞이 대표. 첫 등장에 괄호로 한 번] {d.곡표기.replace(chr(10), " · ") or "없음"}
[주제] {o.주제}
...
- 표기: 아티스트는 [아티스트 표기]대로 첫 등장에 한 번 적고(WOODZ(우즈)) 그 뒤로는 괄호 앞 이름만 쓴다(WOODZ). 괄호 안 이름을 홀로 쓰지 않는다. 앨범 이름은 《 》, 곡 이름은 ' '로 감싼다. 곡 이름은 [다룰 곡] 표기 그대로다. [곡 표기 짝]에 있는 곡은 첫 등장에 '심연(ABYSS)'처럼 짝을 괄호로 한 번 붙이고 그 뒤로는 앞 이름만 쓴다. 영어 곡 이름을 옮기지 않는다.
- 판종: [판종]에 적힌 이름 하나로 부른다. "EP·미니앨범"처럼 두 이름을 붙여 쓰지 않는다.
...
def 꼴검사(t, 문단, album):
    # v3.5: 글의 꼴. 문장 길이가 다 비슷한가, 문단을 같은 방식으로 여는가, 아티스트 표기가 섞였는가, 곡 짝 병기가 있는가
    걸림 = []
    문장들 = [x.strip() for x in re.split(r"(?<=[.!?…])\s+", t) if len(x.strip()) >= 8]
    if 문장길이편차문턱 and len(문장들) >= 10:
        길이 = [len(x) for x in 문장들]
        평균 = sum(길이) / len(길이)
        편차 = (sum((x - 평균) ** 2 for x in 길이) / len(길이)) ** 0.5
        if 평균 and 편차 / 평균 < 문장길이편차문턱:
            걸림.append(f"문장 길이 단조 (편차 {편차 / 평균:.2f})")
    곡으로연 = sum(1 for p in 문단 if re.match(r"^[‘'\"“]", p))
    if 곡으로연 >= 3:
        걸림.append(f"곡 이름으로 문단 열기 {곡으로연}번")
    if album:
        대표, 다른들 = 표기짝(album)
        for x in 다른들:
            if len(x) < 2:
                continue
            홀로 = len(re.findall(re.escape(x), t)) - len(re.findall(re.escape("(" + x + ")"), t))
            if 홀로 > 0:
                걸림.append(f"아티스트 표기 섞임: {x}")
                break
        for 곡, 짝들 in (album.get("곡짝") or {}).items():
            if 곡 in t and not any(("(" + p + ")") in t for p in 짝들):
                걸림.append(f"곡 표기 병기 없음: {곡}({짝들[0]})")
                break
    return 걸림
...
[주문대로 갔는가]
[주문]의 접근 · 온도 · 시작점 · 배치와 그 뜻을 받는다. 주문대로 안 갔으면 "구조"다.
- 짓궂음인데 웃긴 대목이 하나도 없다. 곡 하나의 대목에서 시작하라고 했는데 발매일로 시작했다. 곡 순서 배치인데 곡마다 같은 꼴이다. 평가인데 판단이 마지막 문단에만 있다.
- 시작점대로 열었는지는 첫 문단에서 본다. 배치대로 갔는지는 문단 순서에서 본다.

[되풀이 — 글의 꼴]
- 같은 꼴 문단이 두 번 이상 이어진다(곡 이름 → 소리 묘사 → 감정 해석 → 다음 곡). "구조"다. 곡 순서 배치여도 그렇다.
- 문장 길이가 다 비슷하고 같은 꼴 문장("A는 B한다")이 이어진다. "문장"이다.
- 문단 첫 문장이 같은 방식(곡 이름으로 열기, 《앨범》으로 열기, "이 앨범은")으로 두 번 연속이다. "구조"다.
- "~가 아니라 ~다" 틀이 셋 넘는다. "문장"이다.
- 문단 계획의 할 말 한 줄이 첫 문장에 그대로 보인다. "문장"이다.

[표기표]
[표기표]에 있는 아티스트 짝(WOODZ = 우즈)과 곡 짝(심연 = ABYSS)은 같은 것이다. 어느 쪽을 썼든 사실 문제로 잡지 않는다.
다만 아티스트 표기가 글 안에서 섞이면(첫 등장 괄호 뒤로 두 이름이 번갈아 나오면) "문장"이다. 곡 짝 병기가 없는 것은 잡지 않는다. 코드가 본다.

[앞 판이 있으면]
[앞 판 판정]과 [앞 판 글]이 붙어 오면 [이번 글]은 그 판정을 받고 다시 쓴 글이다. 처음 보는 것처럼 채점하지 않는다. 채점하는 것은 [이번 글]이다. [앞 판 글]은 대조하려고 주는 것이다.
- [이번 글] 머리에 판 종류가 적혀 있다. "새로 씀"이면 앞 판 문제 목록만 받고 처음부터 다시 쓴 것이다. 앞 판 문제가 다른 문장으로 옮겨 와 남았는지 본다. "고침"이면 앞 글을 받아 손본 것이다. 두 글을 문단 단위로 나란히 놓고 바뀐 자리를 본다.
...
def 앞판_text(k):
    """앞 바퀴 기록 하나를 편집국장에게 줄 글로. 점수 · 문제 목록 · 형태 검사 · 한줄평."""
    if not k:
        return ""
    p = k.get("판정")
    ...
    g = k.get("글")
    if g is not None:                                   # v3.4: 앞 판 글 본문. 대조용
        줄 += ["", f"[앞 판 글 — {k.get('차례', '?')}차]", g.제목, "", g.본문]
    return "\n".join(줄)

def 이번글_머리(차례, 종류, 앞판):
    """v3.4: 편집국장에게 주는 [이번 글] 머리. 앞 판이 있을 때만 차례와 판 종류를 붙인다."""
    if not (앞판기억 and 앞판):
        return "[글]"
...
def 표기표_text(album):
    # v3.5: 편집국장에게 주는 표기표. 아티스트 짝 · 곡 짝
    대표, 다른 = 표기짝(album)
    줄 = [f"아티스트: {대표}" + (f" = {' = '.join(다른)}" if 다른 else "")]
    곡 = 곡표기_text(album)
    줄.append("곡: " + (곡.replace(chr(10), " · ") if 곡 else "짝 없음"))
    return chr(10).join(줄)


def 편집국장_읽기(글: Article, 개요_: 개요, 재료, 관점표, 맥락, album, budget, 아티스트말=None, 앞판=None, 차례=None, 종류="", 주문=None):
    if budget["남은콜"] <= 0:
        return None
...
형태_고칠것 = ["존댓말", "번역투 틀", "판단 흐리는 끝맺음", "괄호 인용 표기",
               ...
               "영어 원문",
               "문장 길이 단조", "곡 이름으로 문단 열기", "아티스트 표기 섞임"]   # v3.5. "곡 표기 병기 없음"은 사소
...
_조건자물쇠 = threading.Lock()
_조건판 = {}                       # v3.5: run 한 번 동안 접근 · 온도 · 시작점을 돌려 쓴다. 앞배치는 앞 편이 고른 배치 둘
...
def _조건뽑기():
    if not 조건돌려쓰기:
        return random.choice(접근들), random.choice(온도들), random.choice(시작점들)
    with _조건자물쇠:
        out = []
        for 이름, 목록 in (("접근", 접근들), ("온도", 온도들), ("시작점", 시작점들)):
            남은 = _조건판.get(이름) or []
            if not 남은:
                남은 = list(목록)
                random.shuffle(남은)
...
def n_기획자(st: RunState) -> RunState:
    st.바퀴 += 1
    if st.주문 is None:                                 # v3.5: 같은 앨범을 다시 쓸 때는 조건을 안 바꾼다
        접근, 온도, 시작점 = _조건뽑기()
    else:
    ...
    st.주문 = 기획주문(앨범정보=album_text(st.album), 관점표=st.관점표, 맥락=st.맥락[:5000],
                     ...
                     접근=접근, 온도=온도, 시작점=시작점, 배치후보=" / ".join(배치들),
                     중심곡=st.중심곡 or "", 피할배치=_피할배치(),
    ...
    if st.개요_:
        ...
        if st.중심곡 and not any(같은이름(c, st.중심곡, 느슨=True) for c in o.곡셋):
            o.곡셋 = [st.중심곡] + o.곡셋[:2]
        _배치기록(o.배치)
        st.log(f"  주문: {st.주문.접근} · {st.주문.온도} · {st.주문.시작점} / 배치: {o.배치} / 곡 {len(o.곡셋)}개 ({', '.join(o.곡셋[:5])}) / 사실 {len(o.사실목록)}개" + (f" / 곡 흐름 {len(o.곡흐름)}개" if o.곡흐름 else ""))
        if len(o.사실목록) < 8:
    ...
    try:
        st.글 = 작가.run_sync("쓴다.",
                             deps=집필주문(앨범한줄=f"{표기(a)} - {a['title']} ({a.get('year')})",
                                          ...
                                          접근=st.주문.접근, 시작점=st.주문.시작점, 곡표기=곡표기_text(a)),
...
def n_편집국장(st: RunState) -> RunState:
    ...
    종류 = "첫 판" if not 앞판 else "새로 씀"          # 음악은 편집국장이 걸면 기획자부터 다시 간다. 고침은 없다
    st.판정 = 편집국장_읽기(st.글, st.개요_, st.재료, st.관점표, st.맥락, st.album, b, 아티스트말=st.아티스트말, 앞판=앞판, 차례=st.바퀴, 종류=종류, 주문=st.주문)
    st.남은콜 = b["남은콜"]
    ...
    st.기록.append({"차례": st.바퀴, "종류": 종류, "개요": st.개요_, "글": st.글, "고친곳": st.고친곳,
...
def 저장(st: RunState):
    ...
    최고줄 = []
    if 최고판저장 and not st.올림 and len(st.기록) > 1:      # v3.4: 탈락이면 점수 제일 높은 판 글을 남긴다
    ...
    lines = [f"# {g.제목}", "", g.본문, "", "---", "", "## 앨범",
             ...
             "", "## 출처"] + (출처줄 or ["- 없음"]) + \
            ["", f"## 개요 ({r['차례']}차)", f"- 주제: {r['개요'].주제}", f"- 곡: {', '.join(r['개요'].곡셋) or '-'}",
             "", "## 판정 기록"] + 최고줄 + 로그 + \
...
def run(목표편수=목표편수, 시도상한=시도상한, 쪽만=None, 동시=None):
    ...
    결과, 시도 = [], 0
    시작 = time.time()
    _조건판_새로()                                       # v3.5: 편마다 접근 · 온도 · 시작점을 돌려 쓴다
