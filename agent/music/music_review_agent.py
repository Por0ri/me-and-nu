...
앞판기억       = True   # 편집국장이 앞 판 점수 · 문제 목록을 받는다. 문제마다 고쳐짐 / 남음 / 새로 생김. False면 판마다 처음 보는 것처럼 본다
# ─────────────────────────────────────────────────────────────
...
class 편집판정(BaseModel):
    ...
    문제들: List[문제]
    한줄평: str
    앞판대조: List[str] = Field(default_factory=list, description="앞 판 판정이 있을 때만. 앞 판 문제마다 한 줄. '고쳐짐 — 무엇' / '남음 — 무엇' / 새로 생긴 문제는 '새로 생김 — 무엇'")
...
[앞 판이 있으면]
[앞 판 판정]이 붙어 오면 이 글은 그 판정을 받고 다시 쓴 글이다. 처음 보는 것처럼 채점하지 않는다.
- 앞 판 문제마다 이번 글에서 고쳐졌는지 본다. 앞판대조에 한 줄씩 적는다. "고쳐짐 — 무엇", "남음 — 무엇". 남은 것은 문제들에 다시 넣는다. 고쳐진 것은 다시 잡지 않는다.
- 이번 글에서 새로 생긴 문제는 "새로 생김 — 무엇"으로 적고 문제들에 넣는다.
- 점수는 앞 점수에서 출발한다. 고쳐진 것이 있고 새로 생긴 문제가 없으면 앞 점수보다 낮게 주지 않는다. 새 문제가 생겼거나 고친 자리가 다른 자리를 망가뜨렸으면 내릴 수 있다. 내리면 점수설명에 어느 문제 때문인지 적는다.
...
def 편집국장_읽기(글: Article, 개요_: 개요, 재료, 관점표, 맥락, album, budget, 아티스트말=None, 앞판=None):
    if budget["남은콜"] <= 0:
        return None
...
def n_편집국장(st: RunState) -> RunState:
    ...
    st.판정 = 편집국장_읽기(st.글, st.개요_, st.재료, st.관점표, st.맥락, st.album, b, 아티스트말=st.아티스트말, 앞판=앞판)
    st.남은콜 = b["남은콜"]
    ...
    if 앞판 and st.판정 and 앞판.get("판정"):
        ...
        st.log(f"  앞 판 {앞판['판정'].점수}점 → {st.판정.점수}점 / 고쳐짐 {세기('고쳐짐')} · 남음 {세기('남음')} · 새로 생김 {세기('새로 생김')}" + (f" / {st.판정.점수설명}" if st.판정.점수설명 else ""))
