# # anime_agent v0.3 — 애니 콘텐츠 크리에이팅 에이전트
#
...
유형비율      = {"감상순서": 1, "사람": 1, "기념일": 1, "제작이야기": 1, "작품리뷰": 1}   # run 에 유형을 안 주면 이 비율로 섞는다. 0이면 안 뽑는다
...
[질문과 답]
- 주제는 독자의 질문 한 줄이다. "나루토 감상 순서"가 아니라 "나루토 720화, 다 봐야 하나"다. 물음으로 끝나는 문장이어야 한다.
...
- 글쓴이의 판단이 한두 문장 섞인 것. 정보형에서도 허용한다. 근거가 같은 문장에 있으면 판단은 문제가 아니다. 판단이 글의 중심이 되고 근거가 없을 때만 "구조"다
...
class 개요(BaseModel):
    주제: str = Field(description="독자의 질문 한 줄. 물음으로 끝난다")
...
def 형태검사(본문, st, 제목=""):
    ...
    if 모양 == "정보":
        ...
        재료글 = (st.사실표 or "") + (st.맥락 or "") + (st.이야기 or "")
...
def n_기획자(st: RunState) -> RunState:
    ...
    if st.개요_:
        o = st.개요_
        st.log(f"  주문: {온도} · {st.시작점} · {st.배치}" + (f" · {st.접근}" if st.접근 else "") + f" / 질문: {o.주제[:40]} / 답: {(o.답 or '-')[:40]} / 문단 {len(o.문단들)}개 / 사실 {len(o.사실목록)}개")
        if len(o.사실목록) < 10:
