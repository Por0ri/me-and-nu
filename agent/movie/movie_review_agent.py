# # Movie Review agent V2.0 — 영화 리뷰 에이전트
#
...
#
# **V2.0에서 바뀐 것 — 글이 틀에 박혔다 (PM). 음악 v3.5 · 영화 정보 V2.5와 같은 방식**
#
# - 온도 · 시작점 · 배치 뜻을 한 줄에서 여러 줄로. 작가와 판정관이 같은 [주문](접근 · 온도 · 시작점 · 배치, 뜻 포함)을 받는다(`order_text`).
# - [하나를 잡는다]. 글은 장면 · 선택 · 인물 중 하나를 잡아 끝까지 끌고 간다. 다섯 요소는 그 하나를 지나가며 나온다.
#   요소가 문단 순서와 같으면(줄거리 → 좋은 점 → 아쉬운 점 → 주변 이야기 → 누구에게) 표다.
# - 판단을 이름표로 다는 문장("가장 큰 힘이다", "힘은 분명하다", "매력으로 남는다", "약점으로 느껴진다")을 안 쓴다. 코드도 잡는다.
...
PARA_LENGTH_CV  = 0.15           # 문단 길이의 편차 / 평균이 이 아래면 "문단 길이가 다 비슷하다"로 건다 (문단 넷 이상일 때)
...
[하나를 잡는다]
글은 이 영화에서 네가 잡은 것 하나(장면 하나 · 선택 하나 · 인물 하나)를 끝까지 끌고 간다. 첫 문단에서 잡고 마지막 문단에서 그것으로 돌아온다.
다섯 요소는 그 하나를 지나가며 나온다. 요소가 문단 순서와 같으면(줄거리 → 좋은 점 → 아쉬운 점 → 주변 이야기 → 누구에게) 표다. 글이 아니다.
...
PERSONA = {
"장면부터":    "장면 하나를 먼저 그려 놓고 거기서 시작한다. 그 장면이 왜 거기 있는지가 글 전체다. 마지막 문단에서 그 장면을 한 번 다시 부르며 닫는다. 줄거리를 차례로 옮기지 않는다. 장면은 첫 문단과 마지막 문단에 하나씩이다.",
"대비":        "이 영화 안의 두 가지를 마주 놓고 그 차이로 글을 끌고 간다. 괴물과 가족, 공포와 웃음, 앞 작품과 이 작품, 기대와 실제 같은 것이다. 끝까지 그 둘이 남고, 차이가 판단이 된다.",
"관객":        "이 영화를 보려는 사람이 어디서 막히는지에서 시작한다. 무섭나, 길다는데, 원작을 알아야 하나, 앞 편을 안 봤는데 같은 것이다. 그 막힘을 풀어 주는 것이 글이고, 풀린 자리가 누구에게 맞는지다.",
"맥락":        "이 영화가 어디에서 나왔는지에서 시작한다. 감독의 앞 작품 · 원작 · 시리즈의 자리 중 하나다. 그 자리가 이 영화의 무엇을 정했는지가 글 전체다. 연혁을 읽어 주지 않는다. 앞 작품은 이 영화와 이어지는 이유가 같은 문장에 있을 때만 부른다.",
...
LAYOUT = {
"줄거리 먼저":
    "줄거리를 앞쪽에 놓고 판단을 뒤에서 낸다. 다만 줄거리 문단에도 그 사건이 왜 여기 필요한지 한 문장이 붙는다. 사건만 있는 문단을 만들지 않는다.",
"판단 먼저":
    "이 영화를 어떻게 봤는지를 먼저 드러내고, 줄거리는 그 판단을 받치는 자리에만 쓴다. 첫 문단의 판단은 근거가 같은 문장에 있는 판단이지 이름표가 아니다.",
"줄거리 나눠 넣기":
    "줄거리를 한 덩어리로 적지 않는다. 좋은 점과 아쉬운 점을 말하는 사이사이에 나눠 넣는다. 이야기를 조금 꺼내고, 판단을 붙이고, 다시 이야기로 돌아온다. 이 걸음을 끝까지 반복한다.",
"좋은 점과 아쉬운 점 섞기":
    "좋은 점과 아쉬운 점을 따로 묶지 않는다. 같은 문단 안에서 이어 말한다. 같은 선택이 어디서는 되고 어디서는 안 되는지가 한 문단에 있다. 좋은 점 문단, 아쉬운 점 문단을 따로 두지 않는다.",
"질문에서 출발":
    "이 영화를 두고 할 만한 질문 하나로 시작해서, 근거를 하나씩 놓고 마지막 문단에서 답한다. 마지막 문단 앞에서 \"그래서\", \"결국\"으로 답을 미리 말하지 않는다.",
}


def order_text(order: "WriteOrder") -> str:
    """V2.0: 작가와 판정관이 같은 [주문]을 받는다. 낱말만 넘기지 않고 뜻까지 넘긴다."""
    # 목록에 없는 값이 들어와도(손으로 정한 주문) 터지지 않는다. 뜻 없이 이름만 간다
    return "\n\n".join([
        f"접근 {order.approach} · 온도 {order.temperature} · 시작점 {order.persona} · 배치 {order.layout}",
        APPROACH.get(order.approach, f"[접근 — {order.approach}]").strip(),
...
def build_system_prompt(order: WriteOrder) -> str:
    ...
    parts = [
        ...
        FORM_REVIEW,
        STANCE,
        "[주문 — 판정관도 같은 것을 받는다]\n" + order_text(order),   # V2.0: 접근 · 온도 · 시작점 · 배치를 뜻과 같이
        _length_layer(order),
    ]
...
STRUCTURE_HINTS = ("너무 짧다", "연달아", "끝맺음", "그대로 옮긴", "같은 말로 시작",
                   ...
                   "다 비슷하다", "번 열었다", "이름표로 달았다")   # V2.0: 꼴 문제는 앞 글 없이 다시 쓴다
...
def shape_check(article: Article, order: WriteOrder) -> list[str]:
    ...
    bad += shape_form_check(body, order.material)        # V2.0
    return bad


LABEL_PHRASES = ("가장 큰 힘이", "힘은 분명하", "힘이 분명하", "매력으로 남는다", "약점으로 느껴진다",
                 "답답함을 남긴다", "중심을 잡는다", "선명하게 드러난다")   # 판단이 아니라 판단이 있다는 표시


def shape_form_check(body: str, m: Material) -> list[str]:
    """V2.0: 글의 꼴. 문장 · 문단 길이가 다 비슷한가, 같은 주어로 문단을 여는가, 판단을 이름표로 달았는가."""
    bad = []
    ...
    if PARA_LENGTH_CV and len(paras) >= 4:
        ls = [len(x) for x in paras]
        mean = sum(ls) / len(ls)
        sd = (sum((x - mean) ** 2 for x in ls) / len(ls)) ** 0.5
        if mean and sd / mean < PARA_LENGTH_CV:
            bad.append(f"문단 길이가 다 비슷하다 (편차 {sd / mean:.2f}). 어떤 문단은 짧게 지나가고 어떤 문단은 끝까지 간다")
    ...
    hit = [w for w in LABEL_PHRASES if w in body]
    if len(hit) >= 2:
        bad.append("판단을 이름표로 달았다 (" + ", ".join(hit) + "). 무엇이 어떻게 되는지로 쓴다")
    return bad
...
글 완성도 (1~5)
  ...
  다섯 요소가 문단 순서와 같으면(줄거리 → 좋은 점 → 아쉬운 점 → 주변 이야기 → 누구에게) 표다. 깎는다.
  같은 꼴 문단이 이어지거나 문장 길이가 다 비슷하거나 문단 길이가 다 비슷하면 깎는다.
  판단을 이름표로 단 문장("가장 큰 힘이다", "~는 분명하다", "매력으로 남는다", "약점으로 느껴진다")이 있으면 깎는다.
  [주문]의 접근 · 온도 · 시작점 · 배치대로 안 갔으면 깎는다. 짓궂음인데 웃긴 대목이 없다, 장면부터인데 감독 이름으로 열었다.
