...
`openai:gpt-6-luna`. GPT-6 가 나오면서 같은 자리(가장 싼 층)를 옮겨 탔다. 앞은 `gpt-5.6-luna` 였다.

| | 입력 1M | 캐시 입력 | 출력 1M | 지식 기준일 |
|---|---|---|---|---|
| gpt-5.6-luna (앞) | $0.20 | $0.02 | $1.20 | 2026-02-16 |
| **gpt-6-luna (지금)** | **$0.10** | **$0.01** | **$0.50** | **2026-05-18** |
...
- 판 종류 셋: 첫 판 / 고침(앞 글을 받아 지적된 자리만 손본 것) / 새로 씀(앞 판 문제 목록만 받고 처음부터 다시 쓴 것). 코드가 정한다. 음악은 편집국장이 걸면 기획자부터 가므로 새로 씀뿐이다.
- 내는 것: 앞 판 문제마다 `고쳐짐 / 남음 / 새로 생김 / 앞 판에서 못 봄` 한 줄(앞판대조 · `compared`) + 점수설명 한 줄(`점수설명` · `score_note`).
  "새로 생김"은 글이 나빠진 것이고, "못 봄"은 앞 판 글에도 있었는데 앞 판이 안 잡은 것이다. 못 봄만으로는 점수를 안 내린다.
